...
\end{aligned} B\right) \lambda_i\in K,B\in M_{n,p-1}(K)$$
If $\lambda_1=...=\lambda_n=0$ Applying the induction hypothesis to B (for $S\in G_n(K)\quad SA=\left(S\left(\begin{aligned}
    \lambda_1\\\vdots\\\lambda_n
\end{aligned}\right) B\right)=\left(\begin{aligned}
    0\\\vdots\\0
\end{aligned} B\right)$)

Suppose that $(\lambda_1,...,\lambda_n)\not=(0,...,0)$, By permuting the rows we may assume $\lambda_1\not=0$. As K is division ring, by multiplying the first row by $\lambda_1^{-1}$ , we amy assume $\lambda_1=1$. We add $(-\lambda_i)$ times the first row to the $i^{th}$ row, to reduce A to the form $$\left(\begin{aligned}
...
Let $Cau(X,d)$ be the set of all Cuachy sequences in X. We define a binary relaation $\sim$ on $Cau(X,d)$ as $$(x_n)_{n\in \mathbb{N}}\sim(y_n)_{n\in \mathbb{N}}$$iff$$\lim_{n\rightarrow+\infty}d(x_n,y_n)=0$$
\section{Prop}
