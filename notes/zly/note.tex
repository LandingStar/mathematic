...
$\forall a\in Y,\exists A\in\mathfrak{S}\quad a\in A$. Since A is an initial segment of Y. $A_{<a}=Y_{<a}$. Hence $f(Y_{<a})=f(A_{<a})=a$. Hence $y\in \mathfrak{S}$. Thus Y is the greastest element of $(\mathfrak{S},\subseteq)$.
...
\begin{itemize}
    ...
    \item [(2)]Let X be an infinite set.$$\mathcal{F}_{F_r}(X):=\{A\in \wp(X)\mid X\backslash A\text{is infinite}\}$$ is a filter called the Frechet filter of X.
