...
\begin{itemize}
    ...
    \item [Proof of lemma]$\Rightarrow$ is by definition\\$Leftarrow$:$$\forall x\in V,\exists W_x\in \mathcal{J},x\in W_x\subseteq V.$$ Hence $$V=\bigcap\limits_{x\in V}W-x\in \mathcal{J}$$
    \end{itemize}Let $U\in \mathcal{J}_Y$$$\forall x\in f^{-1}(U)\quad f(x)\in U$$Hence $$U\in \mathcal{V}_{f(x)}$$ Hence there exists an open neighborhood W of $x$ such that $f(W)\subseteq U$ Since $f$ is a mapping , $$W\subseteq f^{-1}(U)$$ Therefore $$f^{-1}(U)\in \mathcal{V}_x$$ Since $x$ is arbitrary, $$f^{-1}(U)\in \mathcal{J}_X$$
