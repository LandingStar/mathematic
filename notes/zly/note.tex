...
\section{Prop}
$i_X(X)$ is dense in $\hat{X}$ (the closure of $i_X(X)$ in $\hat{X}$ in $\hat{X}$ is equal to $i_X(X)$ ($ \hat{X}$))
\subsection*{Proof}
...
Let $y_N=x_{\alpha(N)}^{(N)}$. Without loss of generality , we assume that $$\aleph(0)\leq\alpha(1)\leq...$$
