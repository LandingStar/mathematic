...
\section{Prop}
Let $\Omega$ be a set. And $(g_i)_{i\in I}$ be a family of $\sigma$-algebras on $\Omega$. Then $\bigcap\limits_{i\in I}g_i$ is a $\sigma$-algebra
\subsection*{Proof}
\begin{itemize}
    \item $\forall i\in I$ $$\varnothing\in g_i$$Hence $$\varnothing\in \bigcap\limits_{i\in I}g_i$$
    \item If $A\in \bigcap\limits_{i\in I}g_i$ then $\forall i\in I$ $A\in g_i$ Hence $\forall i\in I$ $$\Omega\setminus A\in g_i$$
    So $$\Omega\setminus A\in \bigcap\limits_{i\in I}g_i$$
    \item Let $(A_n)_{n\in \mathbb{N}}\in (\bigcap\limits_{i\in I}g_i)^\mathbb{N}$. For any $i\in I $$$(A_n)_{n\in\mathbb{N}}\in g_i^\mathbb{N}$$  So $$\bigcap\limits_{n\in \mathbb{N}}(A_n)\in g_i$$so $$\bigcap\limits_{n\in \mathbb{N}}(A_n)\in \bigcap\limits_{i\in I}g_i$$
\end{itemize}
\section{Def}
...
\begin{itemize}
    ...
    \item[(2)] If $g_X$ is a $\sigma$-algebra on $X$ then $f_*(g_X)$ is a $\sigma$-algebra on $Y$
\end{itemize}
\subsection*{Proof}
\begin{itemize}
    ...
    \item [(2)]$$f^{-1}(\varnothing)=\varnothing=g_X$$
    so $$\varnothing\in f_8(g_X)$$
    $\forall B\in f_*(g_X)$ $$f^{-1}(Y\setminus B)=X\setminus f^{-1}(Y)\in g_X$$ so $$Y\setminus B\in f_*(g_X)$$
    $\forall(B_n)_{n\in \mathbb{N}}\in f_*(g_X)^\mathbb{N},B=\bigcup\limits_{n\in \mathbb{N}}B_n$$$f^{-1}(B)=\bigcap\limits_{n\in \mathbb{N}}f^{-1}(B_n)$$So $B\in f_*(g_X)$
\end{itemize}
\section{Def}
Let $(X,g_X)$ and $(Y,g_Y)$ be measurable spaces, $f:X\rightarrow Y$ be a mapping. If $f^{-1}(g_Y)\subseteq g_X$ or equivalently $g_Y\subseteq f_8(g_X)$ (or $\forall B\in g_Y,f^{-1}(B)\in g_X$) then we say that $f$ is measurable.
\section{Prop}
Let $(X,g_X),(Y,g_Y),(Z,g_Z)$ be measurable spaces. $f:X\rightarrow Y$ and $g:Y\rightarrow Z$ be measurable mappings. Then $g\circ f$ is measurable.
\subsection*{Proof}
$\forall B\in g_Z$$$(g\circ f)^{-1}(B)=f^{-1}(g^{-1}(B))$$and $$g^{-1}(B)\in g_Y$$ so $$f^{-1}(g^{-1}(B))\in g_X$$
\section{Def}
...
\section{Corollary}
Let $(X,g_X),(Y,g_Y)$ be measurable spaces. $f:X\rightarrow Y$ be a mapping. $\mathcal{C}_Y\subseteq g_Y$ such that $g_Y=\sigma(\mathcal{Y})$ Then $f$ is measurable iff $$\forall B\in \mathcal{C}_Y\quad f^{-1}(B)\in g_X$$
\subsection*{Proof}
$$\sigma(f)=\sigma(f^{-1}(\mathcal{C}_Y))$$
$f$ is measurable iff $\sigma(f)\subseteq g_X$
\section{Example}
